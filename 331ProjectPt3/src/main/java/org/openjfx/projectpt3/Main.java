...
public class Main {
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 1:
                    ...
                    Student student = new Student(name, SSN, address, email, GPA, contactName, contactNumber,
                            contactAddress);
                    StudentList.add(student);
                    // Student studentTest = StudentList.get(StudentList.size() - 1);
                    // System.out.println(studentTest.name + studentTest.GetSSN() +
                    // studentTest.GetStudentID() + studentTest.GetEmail() +
                    // studentTest.GetAddress() + studentTest.GPA + studentTest.contactName +
                    // studentTest.GetContactPhone() + studentTest.GetContactAddress());
                ...
                case 2:
                    ...
                    // System.out.println(testCourse.courseName + testCourse.prefix +
                    // testCourse.number + testCourse.daysWeek + testCourse.startTime +
                    // testCourse.endTime + testCourse.numCredits);
                ...
                case 3:
                    ...
                    createFaculty(facultyName, facultyEmail, buildingName, officeNum, phoneNumber, facultyDepartment,
                            position);
    ...
    // takes selection for semester and faculty, searches trough the schedule and
    // generates a string of courses
    ...
    // then assigns that to its respective object type from its list
    // searches through schedule for courses and genrates string report based on the
    // semester object that was found
    ...
    // Facilitate the enrollement process of a student into a course for a specific
    // semester
    // takes a student and course selection and passes the objects ref of the
    // student/course to enrollment
    // within the student class where it the enrollment object is created and the
    // student added to the course's student list
    ...
    // take a selection of course,semester, takes their reference from their list,
    // creates a new schedule object with those values
    // this method is used whenever a new course is created in order to maintain the
    ...
    public static void AssignFaculty() {
        ...
        System.out
                .println("Successfully assigned " + faculty.name + " to teach " + course.prefix + " " + course.number);
    ...
    // Creating a new faculty member within the system and associate that faculty to
    // the specific department
    public static void createFaculty(String name, String email, String buildingName, int officeNum, String phoneNum,
    ...
    public static void createCourse(String prefix, int number, String courseName, int daysWeek, String startTime,
            String endTime, int numCredits) {
    ...
    // the corresponding selection is taken from the list and the user is allowed to
    // select which variable to change from the object
    ...
    // searches through the student's enrollment lists and generates a string report
    // of the courses
    ...
    // the user selects a course, that is in the courselist, (based on the index
    // value)
